    ...
    for (int i = 0; i < levelHeight; i++){
      ...
      for (int j = 0; j < levelWidth; j++){
        switch (line.charAt(j)){
          case ' ': tileMap[i][j] = new WallTile(new Hitbox(new PVector(j * tileSize, i * tileSize), new PVector(tileSize, tileSize)), null); break;
          case '#': tileMap[i][j] = new WallTile(new Hitbox(new PVector(j * tileSize, i * tileSize), new PVector(tileSize, tileSize)), new PImage[]{loadImage("sprites/test.png")}); break;
